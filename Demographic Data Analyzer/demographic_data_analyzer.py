...
def calculate_demographic_data(print_data=True):
    ...
    race_no = df['race'].value_counts()
    total_race = df['race'].unique()
    race={}
    for i in total_race:
        race[i]=race_no[i]
    ...
    percentage_bachelors = round((bachelor/len_of_df)*100,1)
    ...
    higher_education = len(df[q1])
    lower_education = len(df[~q1])

    # percentage with salary >50K
    percentage1 = (len(df[q2])/len_of_df)*100

    # What is the minimum number of hours a person works per week (hours-per-week feature)?
    min_work_hours = df['hours-per-week'].min()

    # What percentage of the people who work the minimum number of hours per week have a salary of >50K?
    minmum = df['hours-per-week'].min()
    q1 = df['hours-per-week']==minmum
